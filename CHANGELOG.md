...
### 🛠️ 工具中心卡片式重排（P0 文本提取真正落地）
- **痛点**：原工具中心是「3 个离线按钮 + 书名规则化面板 + 在服工具一句话」拼凑，缺每工具的作用说明、已完成工作量、独立成果入口。
- **改动**：新增 `_tool_center_html()`，把工具中心重排为**卡片网格**，每张卡含 ①作用 ②已完成工作量（实时数字+进度条）③成果入口（🔗 跳对比表/统计页）④运行控制。
  - 顶部「🏆 已落地成果总览」：书名已规则化 / 已抽全文 / 图谱L1 / 待修假摘要 四个数字卡。
  - 离线批处理：提取文本(新增▶全量抽取续跑按钮)、书名规则化、知识图谱L1、元数据补全、摘要修复——均带目的+进度+成果链接。
  - 在服实时：AI分类/AI摘要/在线元数据/媒体转录/媒体摘要——带作用+进度+跳转。
- **配套修复（P0 文本提取真正落地）**：依赖装进运行环境(PyMuPDF/ebooklib/mobi/rarfile/lxml，清华镜像)；续跑查询由「空字符串''(non-null)被当成已提取而跳过」改为按 `text_extracted` 标志；干掉 `length(text_content)<=10` 全量扫描(读 3GB 致 MemoryError)；新增 `tools/run_extract_full.py`(多线程+每本硬超时+可续跑) 绕开服务端单线程卡死。已验证抽取链路通(EPUB 抽出 453 字、落库、标 `text_extracted=1`)。
